...
struct BookingConfirmationView: View {
    ...
    private var rawServiceAmount: Double {
        if let slotPrice = selectedSlot?.price, let value = Double(slotPrice) {
            return value
        }
        if let servicePrice = bookingViewModel.selectedService?.price, let value = Double(servicePrice) {
            return value
        }
        return 0
    }
    ...
    private var discountedTotal: String {
        let discounted = max(rawServiceAmount - bookingViewModel.couponDiscountAmount, 0)
                        ...
                        VStack{
                            ...
                            VStack(alignment: .leading, spacing: 8) {
                                HStack(spacing: 8) {
                                    TextField("Enter promo code", text: $bookingViewModel.couponCode)
                                        ...
                                        .onChange(of: bookingViewModel.couponCode) { _, _ in
                                            if bookingViewModel.isCouponApplied {
                                                bookingViewModel.isCouponApplied = false
                                                bookingViewModel.couponMessage = nil
                                                bookingViewModel.couponDiscountAmount = 0
                                            }
                                            bookingViewModel.couponErrorMessage = nil
                                        }

                                    Button(action: {
                                        Task {
                                            await bookingViewModel.validateCoupon(amount: rawServiceAmount)
                                        }
                                    }) {
                                        Group {
                                            if bookingViewModel.isCouponLoading {
                                                ProgressView()
                                                    .progressViewStyle(CircularProgressViewStyle(tint: .white))
                                            } else {
                                                Text(bookingViewModel.isCouponApplied ? "Applied" : "Apply")
                                    ...
                                    .cornerRadius(theme.cornerRadius)
                                    .disabled(bookingViewModel.isCouponApplied || bookingViewModel.isCouponLoading || bookingViewModel.couponCode.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty)
                                }

                                if let message = bookingViewModel.couponMessage {
                                    Text(message)
                                        .font(theme.typography.regular14)
                                        .foregroundStyle(theme.colors.lightGreen)
                                } else if let error = bookingViewModel.couponErrorMessage {
                                    Text(error)
                                        .font(theme.typography.regular14)
                            ...
                            VStack(alignment: .leading, spacing: 16) {
                                ...
                                VStack(spacing: 12) {
                                    ...
                                    if bookingViewModel.isCouponApplied && bookingViewModel.couponDiscountAmount > 0 {
                                        ChargeRow(
                                            label: "Coupon discount",
                                            value: "-\(formatPrice(String(format: "%.2f", bookingViewModel.couponDiscountAmount)))",
                            ...
                            // Final Price
                            HStack {
                                Text(bookingViewModel.isCouponApplied ? "Total after discount" : "Final Price")
                                    .font(theme.typography.semiBold22)
                                    .foregroundStyle(theme.colors.primary)

                                Spacer()

                                Text(bookingViewModel.isCouponApplied ? discountedTotal : totalPrice)
                                    .font(theme.typography.semiBold22)
                                    .foregroundStyle(theme.colors.primary)
                            }
                            .padding(.horizontal, 20)
